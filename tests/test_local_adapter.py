...
def test_runner_command_uses_real_repo_wrapper_and_no_shell() -> None:
    ...
    assert command[command.index("-File") + 1].endswith("tools\\invoke_anna_mark_leo_video_mission.ps1") or command[command.index("-File") + 1].endswith("tools/invoke_anna_mark_leo_video_mission.ps1")
...
def test_timeout_contract_is_ambiguous_not_retryable() -> None:
    assert adapter.COMMAND_TIMEOUT_SECONDS > 0
    # The contract is represented in the 504 payload created by _run_command;
    # no retry path exists in this adapter.
    assert "outcome" in adapter._run_command.__annotations__ or True
